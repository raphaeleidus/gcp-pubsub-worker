...
import { Message, SubscriberOptions } from '@google-cloud/pubsub/build/src/subscriber';
...
export type WorkHandler = (payload: Buffer, logger: Pino.Logger) => Promise<JobResult>;
...
export default class PubsubWorker extends EventEmitter {
    ...
    constructor(subscriptionName: string, handler: WorkHandler, clientConfig: ClientConfig = {}, subscriberOptions: SubscriberOptions = {}, workerConfig: WorkerConfig = { shutdownGraceperiod: 3000 }) {
        super();
    ...
    shutdown(gracePeriod?: number): Promise<void> {
        ...
        this._isShutdown.then(()=>{
            this.logger.warn({}, "shutdown grace period elapsed without clean shutdown.")
            clearTimeout(timer);
        })
